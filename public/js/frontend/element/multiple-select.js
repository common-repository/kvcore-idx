kvCORE.MultipleSelect = (new function ($, kv, kvSearch) {
	...
	this.multiSelect.prototype = {
		...
		renderDataset: function(dataset) {
			var self = this;
			//renderData is the exact data returns from the typeahead (with all data)
			var renderData = $.extend({}, this.data);
			//datasetVals is just the id part from all the data
			var datasetVals = dataset.map(function(datasetItem) {
				return datasetItem.id.toString();
			});

			//this.vals equals the previous 'area' chosen if exists
			if (!this.vals) {
				this.vals = [];
			}
			...
			var checkedDataset = this.vals.filter(function(val) {
				...
				//does this one match any of the ones in storage sometimes successful
			...
			if (this.id === 'area' || this.id === 'profile-area') {
				...
				//renderData.vals exists if there is an area from previous search
				if (!kv.isEmpty(renderData.vals)) {
					for (var i = 0; i < renderData.vals.length; i++) {
						idHash = kv.getHash(renderData.vals[i], true);
		...
		bindSearch: function() {
			...
			this.$container.find('.kv-multiple-select-search').on('click dblclick keyup', function(e) {
				if (self.tokenDropdownOpened) {
					...
					if (kvCORE.Config.get('usePolygonAreas') === '1') {
	...
	this.multiSelectToken.prototype = $.extend({}, this.multiSelect.prototype, {
		...
		storeSelectedValue: function($input) {
			...
			if (newSelectedValue.length === 1) {
				...
				if (kvCORE.Config.get('usePolygonAreas') === '1' && data) {
