		...
		runMultipleSelect(function() {
			...
			if (data.id === 'area' && kv.isEmpty(request) && !kv.isEmpty(getShortcodeParamsFromListings)) {
				if (!kv.isEmpty(getShortcodeParamsFromListings.area)) {
					//shortcode can pass multiple areas separated by semicolon fix
					if (getShortcodeParamsFromListings.area.indexOf(";") !== -1) {
						var splitArr = getShortcodeParamsFromListings.area.split(";");
						data.vals = splitArr;
					} else {
