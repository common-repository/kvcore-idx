kvCORE.Properties = (new function($, kv, config) {
	...
	function renderBlockTemplates(data) {
		...
		// var shouldScroll = $('#kvcoreidx-properties-page .kv-map-grey-area-filters').length > 0 && kvCORE.Config.get('usePolygonAreas') === '1';
		// if (shouldScroll) {
	...
	this.submitFilterForm = function(obj) {
		...
		if ('undefined' !== typeof(filters.polygonKey) && kvCORE.Config.get('usePolygonAreas') === '1') {
			delete filters.area;
		} else {
	...
	function bindProperties($propertiesContainer, data) {
		...
		$propertiesPageForm.submit(function(e) {
			...
			shortcodeFilters.area = shortcodeFilters['area[]'].join(";");
			delete shortcodeFilters['area[]'];

			if (!kv.isEmpty(shortcodeFilters.actualtypes)) {
				shortcodeFilters.propertyTypes = shortcodeFilters.actualtypes;
				delete shortcodeFilters.actualtypes;
			}
	...
	function getChunkedResults(data, page, DoNotRefreshMapFlag) {
		...
		if (!kv.isEmpty(data.data)) {
			...
			var perpageUserSet = kv.Config.get('options', 'listings', 'per_page' ) !== null ? kv.Config.get('options', 'listings', 'per_page' ) : 24;

			data.last_page = Math.ceil(totalRecords / perpageUserSet);
